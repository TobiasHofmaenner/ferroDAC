...
| **U2** | dimensional routing + charts multi-axis + processor `units_out()` | **DONE** |
| **X1** | `core/uncertainty.py` model types `Abs/Rel/FloorRel/Measured/Spec(random,systematic)` | **DONE** |
| **X2** | `Source(uncertainty=…)` + `set_uncertainty` provenance time-resolution + real drivers | **DONE** (a/b) |
