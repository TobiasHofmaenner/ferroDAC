...
These wrap the existing `ferrodac.store.*_selftest` modules (each exposes
`main() -> int`, 0 = pass) so `pytest` discovers them, gives a clean summary, and
CI can gate on them. They need only numpy + zarr (no Qt, no gRPC) — the durable
store, tiered resolver, replay engine, and store-and-forward sync.
"""
